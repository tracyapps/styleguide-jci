 ...
 *
 * contact info:		start_contact_info();
 * copyright text:		start_copyright_text();
 * social link list:	start_social_links( [li-class-prefix], [link-target], [ul-classes], [ul-id] );
 *
 * or social link custom template:
 *
 * foreach ( start_get_social_links() as $link ) :
 *  // Do something with $link
 * endforeach;
 *
 * google maps API key:	start_google_maps_api_key();  or  start_get_google_maps_api_key();
 * FB app ID:			start_fb_app_id();   or   start_get_fb_app_id();
 *
 */
 ...
 */
function start_get_options_page_url() {
 ...
 */
function start_default_post_thumbnail_html( $html, $post_id, $post_thumbnail_id, $size, $attr ) {
	...
	if ( $default_thumbnail_id = start_get_post_thumbnail_id( $post_id ) ) {
		$html = wp_get_attachment_image( $default_thumbnail_id, $size, false, $attr );
	}

	return $html;
}
add_filter( 'post_thumbnail_html', 'start_default_post_thumbnail_html', 10, 5 );
 ...
 */
function start_has_post_thumbnail( $post = null ) {
	return (bool) start_get_post_thumbnail_id( $post );
}
 ...
 */
function start_excerpt_more( $more_text ) {
 ...
 */
function start_trim_excerpt_characters( $trimmed, $raw_excerpt ) {
...
add_filter( 'wp_trim_excerpt', 'start_trim_excerpt_characters', 10, 2 );
 ...
 */
function start_get_contact_info() {
 ...
 */
function start_contact_info() {
	// Escape output (ACF takes care of wptexturize-ation, shortcodes, etc).
	echo wp_kses_post( start_get_contact_info() );
}
 ...
 *               'handle' may be empty.
 */
function start_get_social_links( $services = false ) {
 ...
 * @return void
 */
function start_social_links( $class_prefix = 'icon-', $link_target = '_blank', $ul_class = 'menu menu-social-links', $ul_id = '', $svg_icons = false ) {

	$social_links = start_get_social_links();
	$ul_id_html = '';
	...
	if ( count( $social_links ) ) {
		...
		echo '<ul class="' . implode( ' ', apply_filters( 'start_social_links_ul_classes', $classes ) ) . '" ' . $ul_id_html . ">";
		...
		foreach ( $social_links as $link ) {
			...
			$li_class = apply_filters( 'start_social_links_li_class', $li_class );
 ...
 */
function start_fb_app_id() {
	echo esc_attr( start_get_fb_app_id() );
}

/**
 * Return the Google Maps API key as specified on the Site Options page.
 */
function start_get_google_maps_api_key() {
...
function add_google_maps_key_to_acf() {
	if ( ! empty( start_get_google_maps_api_key() ) ) {
		acf_update_setting( 'google_api_key', start_get_google_maps_api_key() );
	}
}
